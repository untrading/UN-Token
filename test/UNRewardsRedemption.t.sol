...
contract UNRewardsRedemptionTest is Test {
    ...
    UNRewardsRedemption private redemption;
    ...
    function setUp() external {
        ...
        sablier = ISablierV2LockupLinear(0xAFb979d9afAd1aD27C5eFf4E27226E3AB9e5dCC9); // https://docs.sablier.com/contracts/v2/deployments/v2.1
        ...
        redemption = new UNRewardsRedemption(
            address(token), 
            root,
            0, 
        ...
        token.mint(address(redemption), 2e18);
    ...
    function test_ClaimInstant() external {
        ...
        uint256 streamId = redemption.claim(tree[0].amount, Stake.None, m.getProof(hashedTree, 0));

        assertGt(streamId, 0);
        assertEq(streamId, nextStreamId);
        assertEq(redemption.claimedAmount(address(this)), tree[0].amount);
        assertEq(redemption.getStreamId(address(this))[0], streamId);
    ...
    function test_ClaimTiered() external {
        uint256 nextStreamId = sablier.nextStreamId();
        uint256 streamId = redemption.claim(tree[0].amount, Stake.TierOne, m.getProof(hashedTree, 0));
    ...
    function test_ClaimTier4() external {
        uint256 streamId = redemption.claim(tree[0].amount, Stake.TierFour, m.getProof(hashedTree, 0));
    ...
    function test_SubsequentClaim() external {
        redemption.claim(tree[0].amount, Stake.None, m.getProof(hashedTree, 0));
        ...
        token.mint(address(redemption), 1e18); // Need to re-fund the contract after root update
    ...
    function testRevert_ImproperAmountsShouldRevert() external {
        ...
        redemption.claim(tree[0].amount + 1, Stake.None, proof);

        vm.expectRevert("Invalid proof");
        redemption.claim(tree[0].amount - 1, Stake.None, proof);

        vm.expectRevert("Invalid amount");
        redemption.claim(0, Stake.None, proof);
    }

    function testRevert_AlreadyClaimed() external {
        bytes32[] memory proof = m.getProof(hashedTree, 0);
        redemption.claim(tree[0].amount, Stake.None, proof);
    ...
    function testRevert_NotKYCVerified() external {
        ...
        redemption.claim(tree[1].amount, Stake.None, proof);
    }
    ...
    function testRevert_UnauthorizedRootUpdate() external {
        ...
        vm.expectRevert("UNAUTHORIZED");
        vm.prank(address(0xB0B));
        redemption.updateVesting(10, 8 days);
    }
    ...
    function test_UpdateVesting() external {
        ...
        assertEq(redemption.cliff(), 10);
        assertEq(redemption.vestingPeriod(), 8 days);
    }
}
